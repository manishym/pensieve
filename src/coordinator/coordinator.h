...
// through an outbound TCP connection, optionally using registered buffers
...
public:
    ...
    // Serve one client connection: read requests in a loop, dispatch each
    // locally or to a remote peer, write responses back.  The coroutine
    // takes ownership of `client_fd` and closes it when done.
